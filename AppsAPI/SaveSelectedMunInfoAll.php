...
// Query to fetch rows
$query = "SELECT id, Q4A, DIVISION_CODE, DIVISION_NAME, DISTRICT_CODE, DISTRICT_NAME FROM InstituteInfo WHERE Type='Municipal' AND UserID = ? AND id NOT IN(SELECT SampleHHNo FROM xformrecord WHERE UserID=$userID)";
...
try {
    ...
    $csvFiles = [];
    $headers = ['SL', 'InstID', 'InstName', 'DivCode', 'DivName', 'DistCode', 'DistName'];
    $sl = 1;
