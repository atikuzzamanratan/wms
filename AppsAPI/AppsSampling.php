...
WHERE aftu.UserID = ? AND dcf.id <> ? order by dcf.id ASC";
$rsForm = $app->getDBConnection()->query($qryForm, $UserID, $formIdSamplingData);*/

$qryForm = "SELECT dcf.id, dcf.FormName FROM assignformtoagent aftu
JOIN datacollectionform dcf on dcf.id = aftu.FormID 
WHERE aftu.UserID = ? AND dcf.id = ? order by dcf.id ASC";
$rsForm = $app->getDBConnection()->query($qryForm, $UserID, $formIdMainData);
...
if ($_REQUEST['show'] === 'Show') {
    ...
    $dataURL = $baseURL . "AppsAPI/view-sample-list.php?ui=$UserID&psu=$selectedPSU&FormID=$FormID";
    ?>
