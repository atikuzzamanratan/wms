...
foreach ($queryRS as $row) {
    ...
    //$queryIsCollected = "SELECT count(*) as totalCollected from xformrecord where FormId = ? and UserID = ? and PSU = ? and SampleHHNo = ?";
	//$rowQueryIsCollected = $app->getDBConnection()->fetch($queryIsCollected, $FormID, $UserID, $PSU, $SampleHHNumber);
	
    $queryIsCollected = "SELECT count(*) as totalCollected from xformrecord where FormId = 2 and UserID = $UserID and PSU = $PSU and SampleHHNo = $MainHHNumber";
    $rowQueryIsCollected = $app->getDBConnection()->fetch($queryIsCollected);
    $hasCollected = $rowQueryIsCollected->totalCollected;
    //var_dump($queryIsCollected);
