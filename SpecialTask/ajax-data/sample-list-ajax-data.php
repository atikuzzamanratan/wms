...
if ($_REQUEST['psu'] != '') {
    $PSUID = $app->cleanInput($_REQUEST['psu']);
}

$query = "SELECT s.id, s.XFormID, s.PSU, s.UserID, s.HHeadName, s.MobileNumber, s.HHAddress, s.MainHHNumber, s.SampleHHNumber, s.CompanyID, u.UserName, u.FullName, s.geopoint, pl.DivisionName, pl.DistrictName 
FROM SampleMapping AS s 
	JOIN userinfo AS u ON s.UserID = u.id 
	JOIN PSUList pl ON pl.PSUUserID = u.id AND s.PSU = pl.PSU
WHERE s.UserID = ? AND s.PSU = ?
ORDER BY MainHHNumber, PSU ASC";
$queryRS = $app->getDBConnection()->fetchAll($query, $UserID, $PSUID);
