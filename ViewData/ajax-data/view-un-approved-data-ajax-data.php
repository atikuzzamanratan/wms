...
if (!empty($_POST)) {
	...
	(SELECT TOP 1 mdp.ColumnValue FROM masterdatarecord_UnApproved mdp WHERE mdp.XFormRecordId = xfr.id AND mdp.FormId = xfr.FormId AND mdp.UserID = xfr.UserID AND mdp.CompanyId = xfr.CompanyId AND mdp.SampleHHNo = xfr.SampleHHNo AND mdp.ColumnName = N'surveyEndDate') AS StartTime, (SELECT TOP 1 mdp.ColumnValue FROM masterdatarecord_UnApproved mdp WHERE mdp.XFormRecordId = xfr.id AND mdp.FormId = xfr.FormId AND mdp.UserID = xfr.UserID AND mdp.SampleHHNo = xfr.SampleHHNo AND mdp.ColumnName = N'surveyStartDate') AS EndTime FROM xformrecord xfr JOIN userinfo ui ON xfr.UserID = ui.id JOIN InstituteInfo pl ON pl.id = xfr.SampleHHNo ";
        ...
        $actions .= "<button title=\"$btnTitleNotice\" type=\"button\" class=\"btn btn-outline-secondary\" style=\"width: 100%; display: inline-block;margin: 0 1px;\" data-bs-toggle=\"modal\" data-bs-target=\"#sendNoticeModal$RecordID\"><i class=\"fas fa-bell\"></i></button>
                ...
                <script type=\"text/javascript\">
                    ...
                    function ShowDataDetailForViewOnly(dataFromID,recordID, isAproved, psu, loggedUserID, agentID, XFormsFilePath, data) {
                            $.ajax({
                                url: 'ViewData/ajax-data/data-detail-view-unapproved-data-for-view-only.php',
                                method: 'GET',
                                datatype: 'json',
                                data: {
                                    dataFromID: dataFromID,
