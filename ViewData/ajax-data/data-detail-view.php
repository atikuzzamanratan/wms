...
$ApprovalStatus = xss_clean($_REQUEST['data']);
...
foreach ($ModuleGroupData as $ModuleName => $ModuleData) {
    ...
    $dataViewTable .= getAccordionTableData($ModuleData, $XFormsFilePath, $baseURL, $googleMapApiKey, $LoggedUserName, $EditPermission, $IsEditable, $AgentFullName, $AgentUserName, $IsApproved, $RowGender, $app, $DataFromID);
...
if (strpos($LoggedUserName, 'val') !== false && $IsApproved) {
    $dataViewTable .= "<button type=\"button\" class=\"btn btn-success\" name=\"checked\" id=\"checked\" value=\"Checked\" 
                        onclick= \"CheckDataRecord('$RecordID', '$AgentID', '$LoggedUserID');\"> Mark as Checked</button>";
}
if ((strpos($LoggedUserName, 'admin') !== false) or $EditPermission == 1 or (strpos($LoggedUserName, 'dist') !== false) or (strpos($LoggedUserName, 'val') !== false && $IsApproved)) {
...
function getAccordionTableData($ModuleData, $XFormsFilePath, $baseURL, $googleMapApiKey, $LoggedUserName, $EditPermission, $IsEditable, $AgentFullName, $AgentUserName, $IsApproved, $RowGender, $app, $formId) {
    $dataViewTable = '';
    foreach ($ModuleData as $MDRrow) {
        ...
        if (strtoupper($originalColumnName) == 'PICTURE' or strtoupper($originalColumnName) == 'IMAGE' or (strpos(strtoupper($originalColumnName), "IMAGE") !== false) or (strpos(strtoupper($originalColumnName), "PICTURE") !== false)) {
            ...
            $dataViewTable .= "
            <td style=\"word-wrap: break-word;".($Gender==2 ? 'background-color:#ead1dc;' : '')."\">$ModifiedColumnValue</td>";
        ...
        if (strpos($ModifiedColumnLabel, "geopoint") === false 
                ...
                && $IsEditable != 0 
                && ($EditPermission == 1
                || (strpos($LoggedUserName, 'val') !== false && $IsApproved)
                || strpos($LoggedUserName, 'sasadmin') !== false)) {
    
            preg_match_all("/\((.*?)\)/", $ModifiedColumnLabel, $matches);
            $fieldName = $matches[1][count($matches[1])-1];
    
            $dataViewTable .= "<td ".(($IsEdited == 1) ? 'style="background-color: #FBC6C2;"' : (($IsEdited == 2) ? 'style="background-color: #fff192;"' : ($Gender==2 ? 'style="background-color:#ead1dc;"' : '')))."><input type='text' class='form-control' name='field[$fieldName]'></td>";
        }
