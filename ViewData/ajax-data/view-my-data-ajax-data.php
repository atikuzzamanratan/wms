...
if (!empty($_POST)) {
	...
	$qry .= "WHERE xfr.FormId = $DataFromID 
				AND xfr.CompanyId = $DataCompanyID 
				AND xfr.ValidatorID = $LoggedUserID";
    ...
    if (!empty($request['search']['value'])) {
		...
		if(stripos("pending", strtolower($request['search']['value'])) !== false){
			$qry .= " OR xfr.IsApproved = 0";
		} elseif(stripos("approved", strtolower($request['search']['value'])) !== false){
			$qry .= " OR (xfr.IsApproved = 1 AND xfr.IsChecked = 0)";
		} elseif(stripos("checked", strtolower($request['search']['value'])) !== false){
			$qry .= " OR (xfr.IsApproved = 1 AND xfr.IsChecked = 1)";
		} elseif(stripos("un-approved", strtolower($request['search']['value'])) !== false){
			$qry .= " OR xfr.IsApproved = 2";
		} elseif(stripos("deleted", strtolower($request['search']['value'])) !== false){
			$qry .= " OR xfr.IsApproved = 3";
		}
...
//die("SQL: ".$qry);
    ...
    foreach ($resQry as $row) {
		...
		if ($IsApproved == 1 AND $row->IsChecked == 1) {
			$DataStatus = "Checked";
		}
		...
		switch ($row->IsApproved) {
			case 0:
				if ($row->PendingStartTime != NULL) {
					$start = strtotime($row->PendingStartTime);
					$end = strtotime($row->PendingEndTime);
					try {
						$start_date = new DateTime($row->PendingStartTime);
					} catch (Exception $e) {
					}
					try {
						$since_start = $start_date->diff(new DateTime($row->PendingEndTime));
					} catch (Exception $e) {
				...
				break;
			case 1:
				if ($row->ApprovedStartTime != NULL) {
					$start = strtotime($row->ApprovedStartTime);
					$end = strtotime($row->ApprovedEndTime);
					try {
						$start_date = new DateTime($row->ApprovedStartTime);
					} catch (Exception $e) {
					}
					try {
						$since_start = $start_date->diff(new DateTime($row->ApprovedEndTime));
					} catch (Exception $e) {
				...
				break;
			case 2:
				if ($row->UnApprovedStartTime != NULL) {
					$start = strtotime($row->UnApprovedStartTime);
					$end = strtotime($row->UnApprovedEndTime);
					try {
						$start_date = new DateTime($row->UnApprovedStartTime);
					} catch (Exception $e) {
					}
					try {
						$since_start = $start_date->diff(new DateTime($row->UnApprovedEndTime));
					} catch (Exception $e) {
        ...
        $actions = "<div style= \"display: flex; align-items: center; justify-content: center;\">
                ...
                <script type=\"text/javascript\">
                    function ShowDataDetail(dataFromID, recordID, isAproved, psu, loggedUserID, agentID, XFormsFilePath, data) {
                            $.ajax({
                                url: 'ViewData/ajax-data/data-detail-view-my-data.php',
                                method: 'GET',
                                datatype: 'json',
                                data: {
